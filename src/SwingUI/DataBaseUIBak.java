...
public class DataBaseUIBak extends JFrame {
	...
	public DataBaseUIBak() throws ClassNotFoundException, SQLException {
		...
		JLabel lblNewUin = new JLabel("New UID");
		...
		JLabel lblUin_1 = new JLabel("UID");
		...
		JLabel lblUin_2 = new JLabel("UID");
		...
		chckbxBooks.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				...
				if(e.getStateChange() == ItemEvent.SELECTED){
					...
					int end = attributes_Book.length;
		...
		JLabel lblGroupBy = new JLabel("Order By");
				...
				
				{
					...
					if(listmodel_1.isEmpty())
						...
						for(int i = 0; i < listmodel_1.getSize(); i++)
							if(i == 0) query += listmodel_1.getElementAt(i);
							else query += ", " + listmodel_1.getElementAt(i);
					
					query += " FROM ";

					String tables = "";
					if(chckbxBorrowers.isSelected())
						tables = chckbxBorrowers.getText();
					if(chckbxPeople.isSelected())
						if(tables.isEmpty())
							tables = chckbxPeople.getText();
						else
							tables += " NATURAL JOIN " + chckbxPeople.getText();
					...
					if(!textArea.getText().isEmpty() && comboBox_12.getSelectedIndex() != 0 && !textField_12.getText().isEmpty())
						query += " WHERE " + textArea.getText() + String.valueOf(comboBox_12.getSelectedItem()) + textField_12.getText();
					
					if(!listmodel_groupby_2.isEmpty()) {
						query += " ORDER BY ";
						for(int i = 0; i < listmodel_groupby_2.getSize(); i++)
							if(i == 0) query += listmodel_groupby_2.getElementAt(i);
...
//SELECT DISTINCT(Name) FROM Books WHERE IsAvailable=\"Yes\"